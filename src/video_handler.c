...
static ERROR_T  vh_init_video_decoder(CodecDataPacket* data, void* extradata, int extra_len);
static ERROR_T  vh_init_video_context(int width, int height, int framerate, void* extradata, int extra_len);

/* *** FFMPEG DECODE FRAME( WITH OVERLAY UPDATE ) *** */
static ERROR_T  decode_video_frame(AVPacketPacket* packet, void* payload);
static ERROR_T  decode_send(AVPacketPacket* packet, void* payload);
static ERROR_T  decode_receive(AVFrame *frame, AVFrame *hw_frame);

/* *** FFMPEG HANDLE VIDEO MESSAGE *** */
static void handle_video_codec(CodecDataPacket* packet, void* payload);
static void handle_video_data(AVPacketPacket* packet, void* payload);
static int  handle_video_disconnect(void);

/* *** THREAD *** */
static void* vh_thread_video_handler(void *arg);
 ...
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int vh_init_handler_thread(void)
{
    ERROR_T status;
    int thread_id = -1;
    ...
    thread_id = pthread_create(&thread_vh, NULL, vh_thread_video_handler, NULL);
...
static ERROR_T vh_init_video_decoder(CodecDataPacket* packet, void* extradata, int extra_len)
{
    ...
    return vh_init_video_context(packet->video.width, packet->video.height, packet->video.framerate,
                                extradata,  extra_len);
}

int vh_send_message(void* msg, int message_type)
{
...
static ERROR_T decode_send(AVPacketPacket* packet, void* payload)
{
    ...
    //memcpy(av_packet.data, payload, packet->hdr.payloadSize);
    ...
    //printf("avcodec_is_open(video_codec_context) = %d\n", avcodec_is_open(video_codec_context));
    //printf("avcodec_is_Decoder(video_codec_context) = %d\n", av_codec_is_decoder(video_codec_context));
    //printf("codec_type = %d\n", video_codec_context->codec_type);
...
static ERROR_T decode_receive(AVFrame *frame, AVFrame *hw_frame)
{
    ...
    while(ret >= 0)
    {
        ...
        if (tmp_frame->key_frame)
        {
            if((prevWidth != tmp_frame->width) || (prevHeight != tmp_frame->height))
            {
#ifdef __RK_HW_DECODER__
                dh_display_init_video_overlay(tmp_frame->width, tmp_frame->height, tmp_frame->format, 0);
#else
                dh_display_init_video_overlay(tmp_frame->width, tmp_frame->height, AV_PIX_FMT_YUV420P, 0);
#endif
        ...
        printf("Pixel Format: [ %3d ] Key Frame: [ %d ] Resolution [ %dx%d ] preview Rsolution [ %dx%d ]\n", \
                tmp_frame->format, tmp_frame->key_frame, tmp_frame->width, tmp_frame->height, prevWidth, prevHeight);
...
static ERROR_T decode_video_frame(AVPacketPacket* packet, void* payload)
{
    ...
    /* send to packet data to decoder */
    ret = decode_send(packet, payload);

    /* receive frame data from decoder */
    ret = decode_receive(frame, hw_frame);
...
#endif
    if (vh_init_video_decoder(packet, payload, packet->hdr.payloadSize) < 0)
    {
...
static void handle_video_data(AVPacketPacket* packet, void* payload)
{
    ...
    //decode frame using allocated context
    decode_video_frame(packet, payload);
